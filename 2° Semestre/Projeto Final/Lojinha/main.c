    ...
    do
    {
        ...
        if (opcao != 5)
        {
                ...
                if (sqlite3_step(stmt) == SQLITE_ROW){
                    ...
                    if (qtd <= sqlite3_step(stmt)){
                        ...
                        sqlite3_prepare(db, sql, -1, &stmt, NULL);
                            ...
                            sprintf(sql, "SELECT (quant*preco) AS 'VALOR DA VENDA ATUAL:' FROM vendas WHERE idVenda = %i", ident);
                            sqlite3_prepare(db, sql, -1, &stmt, NULL);
                            printf("\n");
                            ExibirTabela(stmt);
            ...
            case 4:
            {
                ...
                //incrementar a qnt
                sprintf(sql, "UPDATE estoque SET (quant = quant + %i WHERE codigo = %i) AND (preco = %i)", qtd, cod, preco);
                sqlite3_prepare(db, sql, -1, &stmt, NULL);
                sqlite3_step(stmt);
