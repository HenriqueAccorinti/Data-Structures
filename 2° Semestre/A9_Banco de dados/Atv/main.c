/*Isabelle Franchi Dinardi   RA:20.00364-0
 Henrique Fortuna Accorinti  RA:20.00080-4
 Mateus D'angelo             RA:20.00494-0
    ...
    do
    {
        ...
        if (opcao != 0)
        {
            ...
            case 4:
            {
                ...
                sprintf(sql,"SELECT (AVG(tmax)+AVG(tmin))/2 AS 'Temperatura Media' FROM Medicoes WHERE data BETWEEN '%s' AND '%s'", dataIni, dataFim);
                printf("\n%s\n\n", sql); // POPPA
                sqlite3_prepare(db, sql, -1, &stmt, NULL);
                ExibirTabela(stmt);
                break;
